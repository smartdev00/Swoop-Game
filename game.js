    ...
    Game.prototype = {
        ...
        onPickedCollectible: function (collectible) {
            ...
            if (!this.inBonusMode) {
                ...
                var firebaseConfig = {
                apiKey: "AIzaSyAUlu80tIkrp3lbiAiy9yfZeE8c5hGTOtE",
                authDomain: "pdoge-kombat.firebaseapp.com",
                projectId: "pdoge-kombat",
                storageBucket: "pdoge-kombat.appspot.com",
                messagingSenderId: "789024408485",
                appId: "1:789024408485:web:c5132487fa646586d75e4c"
                };
                ...
                // After firebase-app.js is loaded, load Firestore
                loadScript("https://www.gstatic.com/firebasejs/8.10.0/firebase-firestore.js", function() {
                    ...
                    // After both scripts are loaded, initialize Firebase and Firestore
                    initializeFirebase();
                });
                });
            ...
            if (!userTgID) {
              ...
              // Access the player's score from global variables (points earned in this session)
              const sessionScore = points;
            
              // Check if Firestore (window.db) is initialized
              if (window.db) {
                const userDocRef = window.db.collection("telegramUsers").doc(userTgID.toString());
            
                // First, get the current balance from Firestore
                userDocRef.get()
                  .then(function(doc) {
                    if (doc.exists) {
                      const currentBalance = doc.data().balance || 0; // Get current balance or default to 0
                      
                      // Log values for debugging
                      console.log("Current balance in Firestore:", currentBalance);
                      console.log("Session score:", sessionScore);
            
                      // Ensure sessionScore is not being accumulated incorrectly
                      const newBalance = currentBalance + sessionScore;
            
                      // Log the new calculated balance
                      console.log("New balance to update in Firestore:", newBalance);
                      ...
                      .then(function() {
                        ...
                        // Reset the session score after it's added
                        points = 0;
            
                        // Dispatch a custom event to notify balance update success
                        window.dispatchEvent(new MessageEvent("message", { data: "balanceUpdated" }));
                      })
                      .catch(function(error) {
                        console.error("Error updating balance:", error);
            
                        // Dispatch a custom event to notify balance update failure
                        window.dispatchEvent(new MessageEvent("message", { data: "balanceUpdateFailed" }));
                      });
                    } else {
